...
public class P06_FilterStudentsByPhone {
    private static final String[] PHONE_CODES = {"02", "+3592"};
    
    public static void main(String[] args) throws IOException{
        LinkedList<Student> studentsList = getStudents();
    
        Predicate<Student> filterByPhoneNumber = st ->{
            for (String code : PHONE_CODES) {
                if (st.getPhoneNumber().startsWith(code)){
                    return true;
                }
            }
        ...
        Consumer<Student> print = st ->
                System.out.println(String.format("%s %s %s",
                        st.getFirstName(), st.getLastName(), st.getPhoneNumber()));
    
        studentsList.stream()
                .filter(filterByPhoneNumber)
                .forEach(print);
    }
