...
public class P05_FilterStudentsByEmailDomain {
    private static final String FILTER_BY_DOMAIN = "@gmail.com";
    
    public static void main(String[] args) throws IOException {
        LinkedList<Student> studentsList = getStudents();
    
        Predicate<Student> filterByEmailDomain = st -> st.geteMail().endsWith(FILTER_BY_DOMAIN);
        
        Consumer<Student> print = st ->
                System.out.println(String.format("%s %s %s",
                        st.getFirstName(), st.getLastName(), st.geteMail()));
