...
public class P03_StudentsByAge {
    private static final Integer START_AGE_INTERVAL = 18;
    private static final Integer END_AGE_INTERVAL = 24;
    public static void main(String[] args) throws IOException{
        LinkedList<Student> studentsList = getStudents();
    
        Predicate<Student> filterByAgeInterval = st -> START_AGE_INTERVAL <= st.getAge() &&
                st.getAge() <= END_AGE_INTERVAL;
        
        Consumer<Student> print = st ->
                System.out.println(String.format("%s %s %d",
                        st.getFirstName(), st.getLastName(), st.getAge()));
    
        studentsList.stream()
                .filter(filterByAgeInterval)
                .forEach(print);
    }
